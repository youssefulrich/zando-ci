...
export default function CreerBoutiquePage() {
  ...
  async function handleSubmit() {
    if (!data.name || !data.category || !data.city) {
      setError('Nom, catégorie et ville sont requis')
      return
    }
    setLoading(true)
    setError('')
    ...
    if (logoFile) {
      ...
      const { error: uploadError } = await (supabase.storage as any).from('shops').upload(path, logoFile)
      if (!uploadError) {
        const { data: urlData } = (supabase.storage as any).from('shops').getPublicUrl(path)
        logoUrl = urlData.publicUrl
      }
    }

    const { data: shop, error: shopError } = await (supabase as any).from('shops').insert({
      owner_id: user.id,
      name: data.name,
    ...
    <div style={{ background: '#0a0f1a', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '48px 20px 80px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ background: '#111827', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: 24 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <div style={{ width: 72, height: 72, borderRadius: 16, overflow: 'hidden', background: '#1a2236', border: '0.5px solid rgba(255,255,255,0.08)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {logoPreview ? <img src={logoPreview} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 28, color: 'rgba(255,255,255,0.15)' }}>🏪</span>}
              </div>
              <label style={{ padding: '10px 18px', background: 'rgba(34,211,165,0.08)', border: '0.5px solid rgba(34,211,165,0.2)', borderRadius: 10, cursor: 'pointer', fontSize: 13, color: accent, fontWeight: 600 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {CATEGORIES.map(c => (
                  <button key={c.value} onClick={() => set('category', c.value)} style={{
                    padding: '10px 12px', borderRadius: 10, cursor: 'pointer', textAlign: 'left', fontSize: 13,
          ...
          {/* Gratuit */}
          <div style={{ background: 'rgba(34,211,165,0.05)', border: '0.5px solid rgba(34,211,165,0.15)', borderRadius: 12, padding: '14px 18px' }}>
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', lineHeight: 1.6 }}>
              🎉 <strong style={{ color: '#22d3a5' }}>Gratuit pour toujours</strong> — Ouvrez votre boutique sans frais. Un abonnement premium sera disponible prochainement avec des fonctionnalités avancées.
            </p>
          </div>
          ...
          <button onClick={handleSubmit} disabled={loading} style={{
            width: '100%', padding: '16px', background: loading ? 'rgba(34,211,165,0.4)' : accent,
            color: '#0a1a14', borderRadius: 14, border: 'none', fontSize: 15, fontWeight: 800,
            cursor: loading ? 'not-allowed' : 'pointer', letterSpacing: -0.3,
          }}>
            {loading ? 'Création en cours...' : 'Créer ma boutique gratuite →'}
          </button>
        </div>
      </div>
    </div>
