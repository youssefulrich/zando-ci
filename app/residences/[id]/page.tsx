...
export default function BookingFormResidence({
  ...
  function getPrice(): number {
    ...
    if (pricePerMonth && nights >= 28) return Math.round(pricePerMonth * (nights / 30))
    if (pricePerWeek && nights >= 7) return Math.round(pricePerWeek * (nights / 7))
      ...
      <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(34,211,165,0.2)', borderRadius: 20, padding: 24 }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <button onClick={() => setGuests(g => Math.min(maxGuests, g + 1))} style={{ width: 40, height: 40, borderRadius: 8, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: '#e2e8f0', fontSize: 18, cursor: 'pointer', flexShrink: 0 }}>+</button>
        ...
        <button
          ...
          style={{
            ...
            background: (loading || !checkIn || !checkOut || nights <= 0) ? 'rgba(34,211,165,0.3)' : 'linear-gradient(135deg, #22d3a5, #0891b2)',
