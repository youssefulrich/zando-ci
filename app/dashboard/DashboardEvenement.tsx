    ...
    <div style={d.page}>
      ...
      <div style={d.wrap}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 40 }}>
          ...
          <Link href="/publier/evenement" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 22px', background: d.accent, color: '#1a0a3d', borderRadius: 12, fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>
            + Nouvel événement
          </Link>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 32 }}>
          {[
            ...
            <div key={i} style={{ ...d.card, padding: 24, background: stat.highlight ? 'rgba(34,211,165,0.06)' : '#111827', border: stat.highlight ? '0.5px solid rgba(34,211,165,0.2)' : '0.5px solid rgba(255,255,255,0.08)' }}>
          ...
          <div style={{ ...d.card, padding: '20px 24px', marginBottom: 32, display: 'flex', alignItems: 'center', gap: 32 }}>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ height: 8, background: 'rgba(255,255,255,0.06)', borderRadius: 4, overflow: 'hidden' }}>
                <div style={{ height: '100%', borderRadius: 4, background: '#22d3a5', width: `${Math.round((stats.revenus_nets / (stats.revenus_nets + stats.commission)) * 100)}%`, transition: 'width 0.5s ease' }} />
              </div>
            </div>
          ...
          <div style={{ ...d.card, padding: 24 }}>
            ...
            {events.length > 0 ? (
                    ...
                    <div key={e.id} style={{ background: 'rgba(255,255,255,0.03)', border: '0.5px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: 14 }}>
                      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                        <div style={{ width: 48, height: 48, borderRadius: 10, overflow: 'hidden', background: '#1a2236', flexShrink: 0 }}>
                          {(e.main_photo || e.cover_image) ? <img src={e.main_photo || e.cover_image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, color: 'rgba(255,255,255,0.15)' }}>◉</div>}
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flexShrink: 0 }}>
                          <Link href={`/events/${e.id}`} style={{ fontSize: 11, color: d.accent, padding: '4px 10px', borderRadius: 8, border: `0.5px solid ${d.accentBorder}`, background: d.accentBg, textDecoration: 'none', textAlign: 'center' }}>Voir</Link>
                          {!isPast && <Link href={`/modifier/evenement/${e.id}`} style={{ fontSize: 11, color: '#22d3a5', padding: '4px 10px', borderRadius: 8, border: '0.5px solid rgba(34,211,165,0.2)', background: 'rgba(34,211,165,0.08)', textDecoration: 'none', textAlign: 'center' }}>Modifier</Link>}
                          {!isPast && <button onClick={() => deleteEvent(e.id)} style={{ fontSize: 11, color: '#f87171', padding: '4px 10px', borderRadius: 8, border: '0.5px solid rgba(248,113,113,0.2)', background: 'rgba(248,113,113,0.08)', cursor: 'pointer' }}>Retirer</button>}
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
                        <span style={{ fontSize: 10, fontWeight: 600, padding: '3px 8px', borderRadius: 20, background: isPast ? 'rgba(255,255,255,0.05)' : remaining === 0 ? 'rgba(248,113,113,0.15)' : 'rgba(34,211,165,0.1)', color: isPast ? 'rgba(255,255,255,0.3)' : remaining === 0 ? '#f87171' : '#22d3a5', border: `0.5px solid ${isPast ? 'rgba(255,255,255,0.08)' : remaining === 0 ? 'rgba(248,113,113,0.2)' : 'rgba(34,211,165,0.2)'}` }}>
                          {isPast ? 'Terminé' : remaining === 0 ? 'Complet' : `${remaining} restants`}
                        </span>
                        <span style={{ fontSize: 10, fontWeight: 600, padding: '3px 8px', borderRadius: 20, background: e.status === 'active' ? 'rgba(34,211,165,0.1)' : e.status === 'pending' ? 'rgba(251,191,36,0.1)' : 'rgba(255,255,255,0.05)', color: e.status === 'active' ? '#22d3a5' : e.status === 'pending' ? '#fbbf24' : 'rgba(255,255,255,0.3)', border: `0.5px solid ${e.status === 'active' ? 'rgba(34,211,165,0.2)' : e.status === 'pending' ? 'rgba(251,191,36,0.2)' : 'rgba(255,255,255,0.08)'}` }}>
              ...
              <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                ...
                <Link href="/publier/evenement" style={{ fontSize: 13, color: d.accent, textDecoration: 'none', padding: '9px 18px', borderRadius: 10, border: `0.5px solid ${d.accentBorder}`, background: d.accentBg }}>Créer mon premier événement</Link>
              </div>
            )}
          </div>
            ...
            {bookings.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {bookings.slice(0, 8).map((b, i) => (
                  <div key={b.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: i < Math.min(bookings.length, 8) - 1 ? '0.5px solid rgba(255,255,255,0.05)' : 'none' }}>
                    <div>
