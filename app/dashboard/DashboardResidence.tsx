...
export default function DashboardResidence({
  profile, userId, showAll = false
}: {
  profile: Profile; userId: string; showAll?: boolean
}) {
  ...
  useEffect(() => {
    ...
    supabase.from('bookings').select('*').eq('item_type', 'residence').order('created_at', { ascending: false })
      .then(async ({ data: all }) => {
        ...
        const mine = all.filter(b => ids.includes(b.item_id))
        setBookings(mine)
        const confirmed = mine.filter(b => b.status === 'confirmed')
        const brut = confirmed.reduce((s, b) => s + b.total_price, 0)
        const com = confirmed.reduce((s, b) => s + (b.commission_amount || Math.round(b.total_price * 0.1)), 0)
  ...
  async function toggleAvailability(id: string, current: boolean) {
    const supabase = createClient()
    await supabase.from('residences').update({ is_available: !current }).eq('id', id)
  ...
  async function deleteResidence(id: string) {
    ...
    const supabase = createClient()
    await supabase.from('residences').update({ status: 'inactive' }).eq('id', id)
    ...
    <div style={{ background: '#0a0f1a', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 48px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 40 }}>
          ...
          <Link href="/publier/residence" style={{
            padding: '12px 22px', background: accent, color: '#0a1a14',
            borderRadius: 12, fontSize: 13, fontWeight: 700, textDecoration: 'none',
          }}>+ Nouvelle résidence</Link>
        </div>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ height: 8, background: 'rgba(255,255,255,0.06)', borderRadius: 4, overflow: 'hidden' }}>
                <div style={{
                  ...
                  width: `${Math.round((stats.revenus_nets / (stats.revenus_nets + stats.commission || 1)) * 100)}%`,
                }} />
              </div>
            </div>
            ...
            {residences.length > 0 ? (
                ...
                {residences.map(r => (
                  <div key={r.id} style={{ display: 'flex', gap: 12, background: 'rgba(255,255,255,0.03)', border: '0.5px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: 12 }}>
                    <div style={{ width: 48, height: 48, borderRadius: 10, overflow: 'hidden', background: '#1a2236', flexShrink: 0 }}>
                      {(r.main_photo || (Array.isArray(r.photos) && r.photos[0]))
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        <span style={{
                          ...
                          background: r.status === 'active' ? 'rgba(34,211,165,0.1)' : 'rgba(251,191,36,0.1)',
                          color: r.status === 'active' ? '#22d3a5' : '#fbbf24',
                          border: `0.5px solid ${r.status === 'active' ? 'rgba(34,211,165,0.2)' : 'rgba(251,191,36,0.2)'}`,
                        }}>{r.status === 'active' ? 'Active' : 'En attente'}</span>
                        {r.status === 'active' && (
                          <span style={{
                            fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 20,
                            background: r.is_available ? 'rgba(96,165,250,0.1)' : 'rgba(248,113,113,0.1)',
                            color: r.is_available ? '#60a5fa' : '#f87171',
                            border: `0.5px solid ${r.is_available ? 'rgba(96,165,250,0.2)' : 'rgba(248,113,113,0.2)'}`,
                          }}>{r.is_available ? 'Disponible' : 'Indisponible'}</span>
                        )}
                      </div>
                    </div>
                    {/* Actions */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flexShrink: 0 }}>
                      <Link href={`/residences/${r.id}`} style={{
                        fontSize: 11, color: accent, padding: '4px 10px', borderRadius: 8,
                        border: '0.5px solid rgba(34,211,165,0.2)', background: 'rgba(34,211,165,0.08)',
                        textDecoration: 'none', textAlign: 'center',
                      }}>Voir</Link>
                      <Link href={`/modifier/residence/${r.id}`} style={{
                        fontSize: 11, color: '#a78bfa', padding: '4px 10px', borderRadius: 8,
                        border: '0.5px solid rgba(167,139,250,0.2)', background: 'rgba(167,139,250,0.08)',
                        textDecoration: 'none', textAlign: 'center',
                      }}>Modifier</Link>
                      {r.status === 'active' && (
                        <button onClick={() => toggleAvailability(r.id, r.is_available)} style={{
                          fontSize: 11, color: '#60a5fa', padding: '4px 10px', borderRadius: 8,
                          border: '0.5px solid rgba(96,165,250,0.2)', background: 'rgba(96,165,250,0.08)',
                          cursor: 'pointer',
                        }}>
                          {r.is_available ? 'Bloquer' : 'Libérer'}
                        </button>
                      )}
                      <button onClick={() => deleteResidence(r.id)} style={{
              ...
              <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                ...
                <Link href="/publier/residence" style={{
                  ...
                  border: '0.5px solid rgba(34,211,165,0.2)', background: 'rgba(34,211,165,0.08)',
                }}>Publier ma première résidence</Link>
              </div>
            )}
          </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {bookings.slice(0, 8).map((b, i) => (
                  <div key={b.id} style={{
                    ...
                    padding: '12px 0', borderBottom: i < 7 ? '0.5px solid rgba(255,255,255,0.05)' : 'none',
                  }}>
                    <div>
