...
export default function DashboardClient({ profile, userId }: { profile: Profile; userId: string }) {
  ...
  return (
    <>
      <style>{`
        .dc-wrap { max-width: 1100px; margin: 0 auto; padding: 40px 48px; }
        .dc-stats { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 32px; }
        .dc-explore { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }
        .dc-booking-row { display: flex; justify-content: space-between; align-items: center; padding: 14px 0; }

        @media (max-width: 767px) {
          .dc-wrap { padding: 24px 16px; }
          .dc-stats { grid-template-columns: 1fr 1fr; }
          .dc-stats > div:last-child { grid-column: span 2; }
          .dc-explore { grid-template-columns: 1fr; }
          .dc-booking-row { flex-direction: column; align-items: flex-start; gap: 8px; }
          .dc-booking-right { display: flex; align-items: center; gap: 12px; }
          .dc-header { flex-direction: column; gap: 12px; }
          .dc-title { font-size: 26px !important; }
        }

        @media (min-width: 768px) and (max-width: 1023px) {
          .dc-wrap { padding: 32px 24px; }
          .dc-explore { grid-template-columns: repeat(3, 1fr); }
        }
      `}</style>
      ...
      <div style={{ background: '#0a0f1a', minHeight: '100vh' }}>
        ...
        <div className="dc-wrap">
          ...
          <div style={{ marginBottom: 40 }}>
            ...
            <h1 className="dc-title" style={{ fontSize: 32, fontWeight: 800, color: '#fff', letterSpacing: -1, marginBottom: 6 }}>
          ...
          <div style={{ ...card, padding: 24 }}>
            ...
            {bookings.length > 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {bookings.map((b, i) => (
                  <div key={b.id} className="dc-booking-row" style={{ borderBottom: i < bookings.length - 1 ? '0.5px solid rgba(255,255,255,0.05)' : 'none' }}>
                    <div>
                    ...
                    <div className="dc-booking-right" style={{ textAlign: 'right' }}>
