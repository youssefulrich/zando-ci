    ...
    <div style={{ background: '#0a0f1a', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 48px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 40 }}>
          ...
          <Link href="/publier/vehicule" style={{ padding: '12px 22px', background: accent, color: '#0a1428', borderRadius: 12, fontSize: 13, fontWeight: 700, textDecoration: 'none' }}>
            + Nouveau véhicule
          </Link>
        </div>
          ...
          <div style={{ ...card, padding: 24 }}>
            ...
            {vehicles.length > 0 ? (
                ...
                {vehicles.map(v => (
                  <div key={v.id} style={{ display: 'flex', gap: 12, background: 'rgba(255,255,255,0.03)', border: '0.5px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: 12 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        <span style={{
                          ...
                          background: v.status === 'active' ? 'rgba(34,211,165,0.1)' : 'rgba(251,191,36,0.1)',
                          color: v.status === 'active' ? '#22d3a5' : '#fbbf24',
                          border: `0.5px solid ${v.status === 'active' ? 'rgba(34,211,165,0.2)' : 'rgba(251,191,36,0.2)'}`,
                        }}>{v.status === 'active' ? 'Actif' : 'En attente'}</span>
                        {v.status === 'active' && (
                          <span style={{
                            fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 20,
                            background: v.is_available ? 'rgba(96,165,250,0.1)' : 'rgba(248,113,113,0.1)',
                            color: v.is_available ? '#60a5fa' : '#f87171',
                            border: `0.5px solid ${v.is_available ? 'rgba(96,165,250,0.2)' : 'rgba(248,113,113,0.2)'}`,
                          }}>{v.is_available ? 'Disponible' : 'En location'}</span>
                        )}
                      </div>
                    </div>
                    {/* Actions */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flexShrink: 0 }}>
                      ...
                      <Link href={`/modifier/vehicule/${v.id}`} style={{
                      ...
                      {v.status === 'active' && (
                        <button onClick={() => toggleAvailability(v.id, v.is_available)} style={{
                          ...
                          {v.is_available ? 'Bloquer' : 'Libérer'}
                        </button>
                      )}
              ...
              <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                ...
                <Link href="/publier/vehicule" style={{ fontSize: 13, color: accent, textDecoration: 'none', padding: '9px 18px', borderRadius: 10, border: '0.5px solid rgba(96,165,250,0.2)', background: 'rgba(96,165,250,0.08)' }}>
