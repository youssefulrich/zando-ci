    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .sp-logo { width: 64px; height: 64px; border-radius: 16px; overflow: hidden; background: rgba(251,146,60,0.15); border: 0.5px solid rgba(251,146,60,0.25); display: flex; align-items: center; justify-content: center; font-size: 20px; font-weight: 800; color: #fb923c; flex-shrink: 0; }
        .sp-desc { font-size: 13px; color: rgba(255,255,255,0.5); line-height: 1.65; margin-top: 12px; display: -webkit-box; -webkit-line-clamp: 4; -webkit-box-orient: vertical; overflow: hidden; }
        @media (min-width: 640px) { .sp-desc { -webkit-line-clamp: unset; overflow: visible; } }
        .sp-actions { display: flex; gap: 8px; flex-wrap: wrap; align-items: center; margin-top: 12px; }
        ...
        <div className="sp-wrap">
          ...
          <div className="sp-header">
            <div className="sp-header-identity">
              ...
              <div style={{ flex: 1 }}>
                <h1 style={{ fontSize: 22, fontWeight: 800, color: '#fff', letterSpacing: -0.5, marginBottom: 4 }}>{shop.name}</h1>
                <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)' }}>📍 {shop.city}{shop.address ? ` · ${shop.address}` : ''}</p>
              </div>
            </div>
            ...
            {isOwner && (
              <div className="sp-owner-btns">
                ...
                <Link href="/modifier/boutique" style={{ padding: '11px 20px', background: 'rgba(96,165,250,0.1)', border: '0.5px solid rgba(96,165,250,0.3)', color: '#60a5fa', borderRadius: 12, fontSize: 13, fontWeight: 700, textDecoration: 'none', whiteSpace: 'nowrap' }}>
                  ✏️ Modifier la boutique
                </Link>
                <Link href="/dashboard" style={{ padding: '11px 16px', background: 'rgba(255,255,255,0.06)', border: '0.5px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.6)', borderRadius: 12, fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>
                  Dashboard
                </Link>
              </div>
            )}
          ...
          {isOwner && (
            <div style={{ background: 'rgba(251,146,60,0.06)', border: '0.5px solid rgba(251,146,60,0.2)', borderRadius: 14, padding: '14px 16px', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <Link href="/publier/produit" style={{ fontSize: 13, color: '#fb923c', textDecoration: 'none', padding: '8px 16px', borderRadius: 10, border: '0.5px solid rgba(251,146,60,0.25)', background: 'rgba(251,146,60,0.08)', fontWeight: 600 }}>+ Produit</Link>
                <Link href="/modifier/boutique" style={{ fontSize: 13, color: '#60a5fa', textDecoration: 'none', padding: '8px 16px', borderRadius: 10, border: '0.5px solid rgba(96,165,250,0.25)', background: 'rgba(96,165,250,0.08)', fontWeight: 600 }}>Modifier</Link>
              </div>
            </div>
          )}
                ...
                return (
                  <Link key={p.id} href={`/boutique/produit/${p.id}`} className="sp-card">
                    ...
                    <div style={{ padding: '12px 14px 14px' }}>
                      <p style={{ fontSize: 13, fontWeight: 600, color: '#f1f5f9', marginBottom: 6, lineHeight: 1.3, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                        {p.name}
                      </p>
