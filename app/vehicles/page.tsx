    ...
    <div style={s.page}>
      ...
      <div style={s.container}>
        ...
        <div style={{ marginBottom: 40 }}>
          ...
          <h1 style={{ fontSize: 36, fontWeight: 800, color: '#fff', letterSpacing: -1, marginBottom: 6 }}>Véhicules disponibles</h1>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 32, flexWrap: 'wrap' }}>
          <Link href="/vehicles" style={{
            ...
            textDecoration: 'none',
            background: !params.type ? '#60a5fa' : 'rgba(255,255,255,0.05)',
            color: !params.type ? '#0a1428' : 'rgba(255,255,255,0.5)',
            border: '0.5px solid rgba(255,255,255,0.1)',
          }}>Tous</Link>
          {TYPES.map(t => (
            <Link key={t} href={`/vehicles?type=${t}`} style={{
              padding: '7px 16px', borderRadius: 20, fontSize: 12, fontWeight: 600,
              textDecoration: 'none', textTransform: 'uppercase',
              background: params.type === t ? '#60a5fa' : 'rgba(255,255,255,0.05)',
              color: params.type === t ? '#0a1428' : 'rgba(255,255,255,0.5)',
              border: '0.5px solid rgba(255,255,255,0.1)',
            }}>{t}</Link>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 32, alignItems: 'flex-start' }}>
          ...
          <aside style={s.sidebar}>
            <form>
              ...
              <label style={s.sideLabel}>Transmission</label>
              <select name="transmission" defaultValue={params.transmission ?? ''} style={s.select}>
                <option value="">Toutes</option>
                <option value="automatique">Automatique</option>
                <option value="manuelle">Manuelle</option>
              </select>

              <label style={s.sideLabel}>Prix / jour (FCFA)</label>
              ...
              <button type="submit" style={{
                width: '100%', padding: 11, background: '#60a5fa',
                color: '#0a1428', borderRadius: 10, border: 'none',
                fontSize: 13, fontWeight: 700, cursor: 'pointer', marginBottom: 10,
              }}>Appliquer</button>
              <Link href="/vehicles" style={{ display: 'block', textAlign: 'center', fontSize: 12, color: 'rgba(255,255,255,0.3)', textDecoration: 'none' }}>
                Réinitialiser
              </Link>
            </form>
          </aside>
            ...
            {vehicles && vehicles.length > 0 ? (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
                  {vehicles.map(v => (
                    <Link key={v.id} href={`/vehicles/${v.id}`} style={{
                      background: '#111827', border: '0.5px solid rgba(255,255,255,0.08)',
                      ...
                      <div style={{ aspectRatio: '4/3', background: '#1a2236', overflow: 'hidden', position: 'relative' }}>
                        ...
                        <span style={{
                          ...
                          border: '0.5px solid rgba(96,165,250,0.25)', textTransform: 'uppercase',
                        }}>{v.type}</span>
                        <span style={{
                          position: 'absolute', top: 12, right: 12,
                          background: v.is_available ? 'rgba(34,211,165,0.15)' : 'rgba(239,68,68,0.15)',
                          color: v.is_available ? '#22d3a5' : '#f87171',
                          fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: 20,
                          border: `0.5px solid ${v.is_available ? 'rgba(34,211,165,0.25)' : 'rgba(239,68,68,0.25)'}`,
                        }}>{v.is_available ? 'Disponible' : 'Loué'}</span>
                        <div style={{ position: 'absolute', bottom: 12, left: 12 }}>
                          <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.7)', margin: 0 }}>{v.city}</p>
                        </div>
                      </div>
                      <div style={{ padding: 16 }}>
                        <p style={{ fontSize: 14, fontWeight: 600, color: '#fff', marginBottom: 4 }}>{v.brand} {v.model}</p>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <p style={{ fontSize: 15, fontWeight: 700, color: '#60a5fa' }}>
                            ...
                            <span style={{ fontSize: 11, fontWeight: 400, color: 'rgba(255,255,255,0.3)' }}> / jour</span>
                          </p>
                ...
                {totalPages > 1 && (
                  <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 40 }}>
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                      <Link key={p}
                        href={`/vehicles?${new URLSearchParams({ ...params, page: String(p) })}`}
                        style={{
                          width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center',
                          borderRadius: 10, fontSize: 13, textDecoration: 'none',
                          background: p === page ? '#60a5fa' : 'rgba(255,255,255,0.05)',
                          color: p === page ? '#0a1428' : 'rgba(255,255,255,0.5)',
              ...
              <div style={{ textAlign: 'center', padding: '80px 20px' }}>
                ...
                <Link href="/vehicles" style={{ display: 'inline-block', marginTop: 20, fontSize: 13, color: '#60a5fa', textDecoration: 'none' }}>
