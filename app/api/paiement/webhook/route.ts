...
async function generateTickets(admin: any, booking: Record<string, unknown>) {
  ...
  if (error) { console.error('[Tickets] Erreur génération:', error); return [] }
  console.log(`[Tickets] ✅ ${ticketsCount} ticket(s) généré(s) pour ${booking.reference}`)
...
async function createNotification(
  ...
  try {
    ...
    console.log(`[Notif] ✅ ${title} → ${userId}`)
  } catch (err) {
    console.error('[Notif] Erreur:', err)
  }
}

// ─── Envoi confirmations (email + notifs in-app) ──────────────────────────────
async function sendConfirmations(
  ...
  try {
    ...
    if (booking.item_type === 'event') {
      ...
      if (evt) {
        ...
        if (customerEmail) {
          ...
          console.log(`[Email] 🎟️ Ticket(s) → ${customerEmail}`)
        }
      }
    ...
    // ── Résidence ─────────────────────────────────────────────────────────────
      ...
      if (res) {
        ...
        if (customerEmail) {
          ...
          console.log(`[Email] 🏠 Résidence → ${customerEmail}`)
        }
    ...
    // ── Véhicule ──────────────────────────────────────────────────────────────
      ...
      if (veh) {
        ...
        if (customerEmail) {
          ...
          console.log(`[Email] 🚗 Véhicule → ${customerEmail}`)
        }
    ...
    await createNotification(
      admin, booking.user_id as string,
      '✅ Paiement confirmé',
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (webhookSecret && signature && timestamp) {
      ...
      if (tsAge > 300) return NextResponse.json({ error: 'Timestamp expiré' }, { status: 400 })
    }

    const payload = JSON.parse(body)
    const event = eventType || payload.event || ''
    const txData = payload.data
    const geniusPayId = String(txData?.id ?? '')
    ...
    if (geniusPayId) {
      ...
      if (existing?.processed) return NextResponse.json({ ok: true, message: 'Déjà traité' })
      if (!existing) {
    ...
    if (event === 'payment.success' || txData?.status === 'completed') {
      ...
      const { data: customerProfile } = await admin.from('profiles').select('full_name').eq('id', booking.user_id).single()

      const customerEmail = authUser?.user?.email ?? ''
      const customerName = customerProfile?.full_name ?? 'Client'

      console.log('[Confirmations] Client:', customerName, '→', customerEmail || 'PAS D\'EMAIL')

      await sendConfirmations(admin, booking, customerName, customerEmail)
