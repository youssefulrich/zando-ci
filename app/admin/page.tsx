...
export default async function AdminPage() {
  ...
  const [
    ...
    // ── BOUTIQUE ──
    { count: shopsCount }, { count: productsCount },
    { data: ordersRaw }, { data: shopsRaw },
  ] = await Promise.all([
    ...
    (supabase as any).from('shops').select('*', { count: 'exact', head: true }),
    ...
    (supabase as any).from('shops').select('id, name, city, logo_url, created_at').order('created_at', { ascending: false }).limit(6),
  ])
  ...
  const usersByType = {
    ...
    seller: allProfiles?.filter((p: any) => p.account_type === 'seller').length ?? 0,
  }

  // Stats boutique
  const orders = (ordersRaw as any[]) ?? []
  const ordersConfirmed = orders.filter(o => ['confirmed','shipped','delivered'].includes(o.status))
  const ordersPending = orders.filter(o => o.status === 'pending').length
  const totalBoutiqueRevenu = ordersConfirmed.reduce((s: number, o: any) => s + (o.total_price ?? 0), 0)
  const totalBoutiqueCommission = ordersConfirmed.reduce((s: number, o: any) => s + (o.commission_amount ?? Math.round((o.total_price ?? 0) * 0.1)), 0)
  ...
  const orderStatusColors: Record<string, string> = { pending: '#fbbf24', confirmed: '#22d3a5', shipped: '#60a5fa', delivered: '#22d3a5', cancelled: '#f87171' }
  const orderStatusLabels: Record<string, string> = { pending: 'En attente', confirmed: 'Confirmée', shipped: 'Expédiée', delivered: 'Livrée', cancelled: 'Annulée' }
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .adm-solde { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; }
        ...
        .adm-boutique-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; }
        .adm-orders-grid { display: grid; grid-template-columns: 1fr; gap: 14px; margin-bottom: 16px; }
        @media (min-width: 640px) {
          ...
          .adm-boutique-grid { grid-template-columns: repeat(4, 1fr); }
        }
        @media (min-width: 1024px) {
          ...
          .adm-orders-grid { grid-template-columns: 1fr 1fr; }
        }
      `}</style>
        ...
        <div className="adm-wrap">
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {totalPendingContact > 0 && <div style={{ background: 'rgba(251,146,60,0.08)', border: '0.5px solid rgba(251,146,60,0.22)', borderRadius: 10, padding: '8px 14px' }}><p style={{ fontSize: 13, fontWeight: 700, color: '#fb923c' }}>📞 {totalPendingContact} contact en attente</p></div>}
              {totalPending > 0 && <div style={{ background: 'rgba(251,191,36,0.08)', border: '0.5px solid rgba(251,191,36,0.22)', borderRadius: 10, padding: '8px 14px' }}><p style={{ fontSize: 13, fontWeight: 700, color: '#fbbf24' }}>⚠️ {totalPending} annonces à valider</p></div>}
              {ordersPending > 0 && <div style={{ background: 'rgba(251,146,60,0.08)', border: '0.5px solid rgba(251,146,60,0.22)', borderRadius: 10, padding: '8px 14px' }}><p style={{ fontSize: 13, fontWeight: 700, color: '#fb923c' }}>🛍️ {ordersPending} commandes boutique en attente</p></div>}
              <div style={{ background: 'rgba(248,113,113,0.07)', border: '0.5px solid rgba(248,113,113,0.18)', borderRadius: 10, padding: '8px 14px' }}><p style={{ fontSize: 12, color: '#f87171', fontWeight: 600 }}>🔐 {profile?.full_name}</p></div>
            </div>
          </div>

          <AdminValidation pendingResidences={pendingResidencesRaw ?? []} pendingVehicles={pendingVehiclesRaw ?? []} pendingEvents={pendingEventsRaw ?? []} />
          ...
          <div style={{ background: 'linear-gradient(135deg, rgba(34,211,165,0.05), rgba(8,145,178,0.03))', border: '0.5px solid rgba(34,211,165,0.18)', borderRadius: 18, padding: '20px', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              ...
              <span style={{ fontSize: 11, color: '#22d3a5', textTransform: 'uppercase', letterSpacing: 2, fontWeight: 700 }}>Solde Zando CI — Réservations</span>
            </div>
          ...
          {/* ══════ MODULE BOUTIQUE ══════ */}
          <div style={{ background: 'linear-gradient(135deg, rgba(251,146,60,0.05), rgba(251,146,60,0.02))', border: '0.5px solid rgba(251,146,60,0.2)', borderRadius: 18, padding: '20px', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#fb923c' }} />
              <span style={{ fontSize: 11, color: '#fb923c', textTransform: 'uppercase', letterSpacing: 2, fontWeight: 700 }}>Marketplace — Boutique</span>
            </div>

            {/* KPIs boutique */}
            <div className="adm-boutique-grid" style={{ marginBottom: 16 }}>
              {[
                { label: 'Boutiques actives', value: shopsCount ?? 0, color: '#fb923c', icon: '🏪' },
                { label: 'Produits publiés', value: productsCount ?? 0, color: '#f1f5f9', icon: '📦' },
                { label: 'Commandes total', value: orders.length, color: '#fbbf24', icon: '🛍️' },
                { label: 'Revenus boutique', value: formatPrice(totalBoutiqueRevenu), color: '#22d3a5', icon: '💰', hl: true },
              ].map((s, i) => (
                <div key={i} style={{ padding: '16px', background: s.hl ? 'rgba(34,211,165,0.06)' : 'rgba(255,255,255,0.02)', border: s.hl ? '0.5px solid rgba(34,211,165,0.15)' : '0.5px solid rgba(255,255,255,0.05)', borderRadius: 12 }}>
                  <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>{s.icon} {s.label}</p>
                  <p style={{ fontSize: 18, fontWeight: 800, color: s.color }}>{s.value}</p>
                </div>
              ))}
            ...
            <div className="adm-orders-grid">
              ...
              <div className="adm-card" style={{ padding: 18 }}>
                <h3 style={{ fontSize: 13, fontWeight: 700, color: '#fff', marginBottom: 14 }}>Commandes par statut</h3>
                {[
                  { label: 'En attente', value: orders.filter((o:any) => o.status === 'pending').length, color: '#fbbf24' },
                  { label: 'Confirmées', value: orders.filter((o:any) => o.status === 'confirmed').length, color: '#22d3a5' },
                  { label: 'Expédiées', value: orders.filter((o:any) => o.status === 'shipped').length, color: '#60a5fa' },
                  { label: 'Livrées', value: orders.filter((o:any) => o.status === 'delivered').length, color: '#22d3a5' },
                  { label: 'Annulées', value: orders.filter((o:any) => o.status === 'cancelled').length, color: '#f87171' },
                ].map((item, i, arr) => (
              ...
              {/* Dernières boutiques */}
              <div className="adm-card" style={{ padding: 18 }}>
                <h3 style={{ fontSize: 13, fontWeight: 700, color: '#fff', marginBottom: 14 }}>Dernières boutiques créées</h3>
                {(shopsRaw as any[])?.length > 0 ? (shopsRaw as any[]).map((s: any) => (
                  <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0', borderBottom: '0.5px solid rgba(255,255,255,0.05)' }}>
                    <div style={{ width: 32, height: 32, borderRadius: 8, overflow: 'hidden', background: 'rgba(251,146,60,0.1)', border: '0.5px solid rgba(251,146,60,0.2)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14 }}>
                      {s.logo_url ? <img src={s.logo_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : '🏪'}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 13, fontWeight: 600, color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.name}</p>
                      <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>{s.city}</p>
                    </div>
                    <span style={{ fontSize: 10, color: '#fb923c', background: 'rgba(251,146,60,0.1)', padding: '2px 8px', borderRadius: 20, flexShrink: 0 }}>
                      {new Date(s.created_at).toLocaleDateString('fr-FR')}
                    </span>
                  </div>
                )) : (
                  <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.3)', textAlign: 'center', padding: '20px 0' }}>Aucune boutique</p>
                )}
              </div>
            </div>
            ...
            {orders.length > 0 && (
              <div className="adm-card" style={{ padding: 18, marginTop: 14 }}>
                <h3 style={{ fontSize: 13, fontWeight: 700, color: '#fff', marginBottom: 14 }}>Dernières commandes boutique</h3>
                <div className="adm-table-wrap">
                  <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 500 }}>
                    <thead>
                      <tr>
                        {['Référence', 'Client', 'Total', 'Statut', 'Date'].map(h => (
                          <th key={h} style={{ textAlign: 'left', fontSize: 10, color: 'rgba(255,255,255,0.25)', textTransform: 'uppercase', letterSpacing: 1, fontWeight: 600, padding: '0 12px 10px 0', borderBottom: '0.5px solid rgba(255,255,255,0.06)', whiteSpace: 'nowrap' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {orders.slice(0, 10).map((o: any) => {
                        const sc = orderStatusColors[o.status] ?? '#fff'
                        return (
                          <tr key={o.id} style={{ borderBottom: '0.5px solid rgba(255,255,255,0.04)' }}>
                            <td style={{ padding: '10px 12px 10px 0', fontSize: 11, fontFamily: 'monospace', color: 'rgba(255,255,255,0.3)', whiteSpace: 'nowrap' }}>{o.reference}</td>
                            <td style={{ padding: '10px 12px 10px 0' }}>
                              <p style={{ fontSize: 12, color: '#e2e8f0' }}>{o.client_name ?? o.buyer_name}</p>
                              {(o.client_phone ?? o.buyer_phone) && <p style={{ fontSize: 10, color: '#60a5fa' }}>{o.client_phone ?? o.buyer_phone}</p>}
                            </td>
                            <td style={{ padding: '10px 12px 10px 0', fontSize: 12, fontWeight: 700, color: '#fb923c', whiteSpace: 'nowrap' }}>{formatPrice(o.total_price)}</td>
                            <td style={{ padding: '10px 12px 10px 0' }}>
                              <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 7px', borderRadius: 20, color: sc, background: `${sc}15`, whiteSpace: 'nowrap' }}>{orderStatusLabels[o.status] ?? o.status}</span>
                            </td>
                            <td style={{ padding: '10px 0', fontSize: 11, color: 'rgba(255,255,255,0.2)', whiteSpace: 'nowrap' }}>
          ...
          <div className="adm-stats3">
            <div className="adm-card" style={{ padding: 20 }}>
              ...
              {[{ label: 'Résidences', value: resCount ?? 0, color: '#22d3a5' }, { label: 'Véhicules', value: vehCount ?? 0, color: '#60a5fa' }, { label: 'Événements', value: evtCount ?? 0, color: '#a78bfa' }, { label: 'Boutiques', value: shopsCount ?? 0, color: '#fb923c' }, { label: 'Produits', value: productsCount ?? 0, color: '#fbbf24' }].map((item, i, arr) => (
                <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: i < arr.length - 1 ? '0.5px solid rgba(255,255,255,0.05)' : 'none' }}>
                  <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>{item.label}</span>
                  <span style={{ fontSize: 16, fontWeight: 800, color: item.color }}>{item.value}</span>
                </div>
            ...
            <div className="adm-card" style={{ padding: 20 }}>
              ...
              {[{ label: 'Résidences', value: byType.residence, color: '#22d3a5' }, { label: 'Véhicules', value: byType.vehicle, color: '#60a5fa' }, { label: 'Événements', value: byType.event, color: '#a78bfa' }].map(item => {
            ...
            <div className="adm-card" style={{ padding: 20 }}>
              ...
              {[
                ...
                { label: 'Vendeurs boutique', value: usersByType.seller, color: '#fb923c' },
              ].map((item, i, arr) => (
                <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: i < arr.length - 1 ? '0.5px solid rgba(255,255,255,0.05)' : 'none' }}>
                  <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.45)' }}>{item.label}</span>
                  <span style={{ fontSize: 16, fontWeight: 800, color: item.color }}>{item.value}</span>
          ...
          <div className="adm-card" style={{ padding: '20px', marginBottom: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
              <h2 style={{ fontSize: 14, fontWeight: 700, color: '#fff' }}>Transactions (réservations)</h2>
          ...
          <div className="adm-card" style={{ padding: '20px' }}>
            ...
            <div className="adm-users">
              {allProfiles?.slice(0, 12).map((p: any) => {
                const utColors: Record<string, string> = { client: '#22d3a5', owner_residence: '#60a5fa', owner_vehicle: '#a78bfa', owner_event: '#fbbf24', seller: '#fb923c' }
                const utLabels: Record<string, string> = { client: 'Client', owner_residence: 'Propriétaire', owner_vehicle: 'Loueur', owner_event: 'Organisateur', seller: 'Vendeur' }
