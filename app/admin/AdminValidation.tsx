...
export default function AdminValidation({ pendingResidences, pendingVehicles, pendingEvents }: Props) {
  ...
  async function approve(table: string, id: string, setter: (fn: (prev: any[]) => any[]) => void) {
    setLoading(id)
  ...
  async function reject(table: string, id: string, setter: (fn: (prev: any[]) => any[]) => void) {
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          background: '#1a2236', flexShrink: 0, position: 'relative',
        }}>
          {photo
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0 }}>
          <button
            onClick={() => approve(table, item.id, setter)}
            disabled={isLoading}
            style={{
          ...
          <button
            onClick={() => reject(table, item.id, setter)}
            disabled={isLoading}
            style={{
          ...
          <Link
            ...
            style={{
              ...
              border: `0.5px solid rgba(${accentRgb},0.2)`,
    ...
    <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: '#92400e' }}>
              {totalPending} annonce{totalPending > 1 ? 's' : ''} soumise{totalPending > 1 ? 's' : ''} par des propriétaires — vérifiez avant de valider
            </p>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 20 }}>
        {TABS.filter(t => t.count > 0 || t.key === 'all').map(t => (
          <button key={t.key} onClick={() => setActiveTab(t.key)} style={{
            ...
            {t.count > 0 && (
              <span style={{
                background: t.key === 'all' ? '#fbbf24' : t.color,
                color: '#0a0f1a', borderRadius: 20,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {visibleResidences.map(r => (
          <Card key={r.id} item={r} table="residences" setter={setResidences} type="residence" />
        ))}
        {visibleVehicles.map(v => (
          <Card key={v.id} item={v} table="vehicles" setter={setVehicles} type="vehicle" />
        ))}
        {visibleEvents.map(e => (
          <Card key={e.id} item={e} table="events" setter={setEvents} type="event" />
        ))}
      </div>
    </div>
  )
